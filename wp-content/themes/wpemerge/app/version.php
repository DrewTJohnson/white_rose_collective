...
if ( ! function_exists( 'daisy_bootstrap_get_loaded_wpemerge' ) ) {
	/**
	 * Get the currently loaded WP Emerge version, if any.
	 *
	 * @return array|false
	 */
	function daisy_bootstrap_get_loaded_wpemerge() {
		/**
...
if ( ! function_exists( 'daisy_bootstrap_should_load_wpemerge' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function daisy_bootstrap_should_load_wpemerge( $name, $min, $max ) {
		$loaded = daisy_bootstrap_get_loaded_wpemerge();
		...
		add_action(
			...
			function () use ( $name, $min, $max, $loaded ) {
				...
				$message = __( '%1$s requires WP Emerge version >= %2$s and < %3$s but %4$s has already loaded version %5$s.', 'daisy_bootstrap' );
				?>
...
if ( ! function_exists( 'daisy_bootstrap_declare_loaded_wpemerge' ) ) {
	/**
	 ...
	 * @return void
	 */
	function daisy_bootstrap_declare_loaded_wpemerge( $name, $type, $file ) {
		$loaded = daisy_bootstrap_get_loaded_wpemerge();
